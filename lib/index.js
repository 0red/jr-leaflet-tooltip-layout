(function(factory, window) {
  ...
  function drawLine(map) {
    ...
    for (var i = 0; i < markerList.length; i++) {
      ...
      var markerPosition = getPosition(markerDom,marker);
   ...
   */
  function getPosition(el,marker) {
    ...
    if (marker._point) return marker._point
    return false;
  }
  ...
  function computePositionStep(t) {
    ...
    for (i = 0; i < markerList.length; i++) {
      v = markerList[i];
      v_pos = getPosition(v.getTooltip()._container);
      dpos = v_pos.subtract(getPosition(v._icon,v));
  ...
  function setEdgePosition() {
    ...
    for (let i = 0; i < markerList.length; i++) {
      ...
      var marker = getPosition(markerList[i]._icon,markerList[i]);
      var width = markerList[i].getTooltip()._container.offsetWidth;
      var height = markerList[i].getTooltip()._container.offsetHeight;
  ...
  TooltipLayout['deleteAllMarker'] = deleteAllMarker;
